...
class FaceapiService {
    ...
    async getDetectorImgFromDetection(el, detection, timeStamp) {
        ...
        const blob = await camera_service.videoToBlob(el, detection.boundingBox);
...
const workerPromise = Promise.resolve().then(function () { return require('./distance.worker-iCnTiTWD.js'); }).then(m => m.worker);
...
const InputFaceApiWebcam = class {
    constructor(hostRef) {
        ...
        this.detectionStarted = index.createEvent(this, "detectionStarted", 6);
        this.detectionStopped = index.createEvent(this, "detectionStopped", 6);
        ...
        this.photoCapture = index.createEvent(this, "photoCapture", 6);
        this.loaded = false;
    ...
    async toggleCamera() {
        try {
            ...
            this.facingMode = this.facingMode === camera_service.CameraDirection.Front ? camera_service.CameraDirection.Rear : camera_service.CameraDirection.Front;
            ...
            await camera_service.initWebcamToVideo(this.video, this.facingMode);
    ...
    async componentWillLoad() {
        try {
            this.cameraState = 'loading';
            this.currentError = null;
            this.video = camera_service.createVideo();
            ...
            const stream = await camera_service.initWebcamToVideo(this.video, this.facingMode);
            this.cameraStarted.emit(stream);
            // Wait for video to be ready before starting render loop
            await this.waitForVideoReady();
            camera_service.renderToCanvas(this.canvas, this.video);
            this.loaded = true;
            if (this.autoStart && this.enableDetection) {
                this.cameraState = 'detecting';
                this.detectionStarted.emit();
    ...
    render() {
        ...
        const renderCameraState = () => {
            ...
            if (this.cameraState === 'inactive') {
                return (index.h("div", { class: "camera-state" }, index.h("div", { class: "state-icon inactive-icon" }, "\uD83D\uDCF7"), index.h("h3", null, "Camera Inactive"), index.h("p", null, "Click start to begin face detection")));
            }
            if (this.cameraState === 'loading') {
                return (index.h("div", { class: "camera-state" }, index.h("div", { class: "loading-spinner" }), index.h("h3", null, "Starting Camera"), index.h("p", null, "Please allow camera access when prompted")));
            }
            if (this.cameraState === 'error') {
                return (index.h("div", { class: "camera-state" }, index.h("div", { class: "state-icon error-icon" }, "\u26A0\uFE0F"), index.h("h3", null, "Camera Error"), index.h("p", null, ((_a = this.currentError) === null || _a === void 0 ? void 0 : _a.message) || 'Unable to access camera')));
            }
            return null;
        };
        const renderIndicators = () => {
            ...
            return [
                ...
                this.autoCapture && (index.h("div", { class: "auto-capture-indicator" }, "AUTO CAPTURE")),
                // Face detection indicator
                this.cameraState === 'detecting' && (index.h("div", { class: `face-indicator ${this.detectedFacesCount > 0 ? 'detected' : 'not-detected'}` }, this.detectedFacesCount > 0 ? `${this.detectedFacesCount} Face${this.detectedFacesCount !== 1 ? 's' : ''} Detected` : 'No Face Detected')),
                // Error message
                this.currentError && (index.h("div", { class: "error-message" }, this.currentError.message))
            ];
        };
        const renderControls = () => {
            ...
            return (index.h("div", { class: "camera-controls" }, this.cameraState === 'inactive' || this.cameraState === 'ready' ? (index.h("button", { class: "control-button start-button", onClick: () => this.startDetection(), disabled: isLoading, title: this.startButtonText }, "\u25B6\uFE0F")) : this.cameraState === 'detecting' ? (index.h("button", { class: "control-button stop-button", onClick: () => this.stopDetection(), title: this.stopButtonText }, "\u23F9\uFE0F")) : null, (this.cameraState === 'ready' || this.cameraState === 'detecting') && (index.h("button", { class: "control-button flip-button", onClick: () => this.toggleCamera(), disabled: isLoading, title: this.flipButtonText }, "\uD83D\uDD04"))));
        };
        return (index.h(index.Host, { style: hostStyle }, index.h("slot", { name: 'before' }), renderCameraState(), renderIndicators(), renderControls(), index.h("slot", null), index.h("slot", { name: 'after' })));
    }
...
exports.InputFaceApiWebcam = InputFaceApiWebcam;
exports.createWorker = createWorker;
//# sourceMappingURL=input-face-api-webcam-n0WJNP3O.js.map
